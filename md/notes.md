...
- ヘッダーのタイトル下にバージョン表示（`v{APP_VERSION} build.{BUILD_NUMBER}`）を追加。現在 build.6
...
- スコア入力欄の視点回転機能を追加（build.6）
  - クリアボタン左に「↻ 回転」ボタンを追加。押すたびに時計回りで視点が切り替わる（東→南→西→北→東）
  - `.rot-0`〜`.rot-3` クラスをグリッドコンテナに付与してCSSで座席の位置を制御
  - 回転ステートに連動してOCRプロンプトの位置説明（`下=東、上=西...`）も動的に変わる
